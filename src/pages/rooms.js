...
import ActionBar from "../components/ActionBarRooms1";
...
componentDidMount(){
  ...
  sortRowsFullName() {
    this.setState({
      roomList: this.props.rooms.sort((a, b) => {
  ...
  sortRowDateaStart() {
    this.setState({
      roomList: this.props.rooms.sort(
        (a, b) =>
  ...
  rowsSearchConfig = () => {
    if (this.state.term === "" ) {
      return this.props.rooms.filter((x)=>new Date(x.start)>=new Date(this.state.dateStart) && new Date(x.end)<=new Date(this.state.dateEnd))
    }
    else {
  ...
  render() {
    ...
    const currentPosts = this.props.rooms.slice(
      indexOfFirstPost,
      indexOfLastPost
    );
    ...
    return (
      <div style={{ maxWidth: "100%" }}>
        ...
        <div className={classes.tablewrapper}>
          ...
          <ActionBar hall={this.props.rooms.filter((x)=>new Date(x.start)>=new Date(this.state.dateStart) && new Date(x.end)<=new Date(this.state.dateEnd))} first={this.state.dateStart}
                ...
                {this.rowsSearchConfig()
                  ...
                  .map((x) => (
                    <tr>
                      ...
                      <td>
                      {new Date(x.end).toLocaleDateString('en-GB')}
                      </td>
                      <td>
...
export default connect(mapStateToProps,{getRooms})(
  Rooms
);
